...
{
    EXPECT_EQ(sut_->getPosition(), board::BEGINNING_POSITION);
}
...
{
    sut_->move();
    auto newPosition = board::BEGINNING_POSITION.getCoordsDecreaseX(board::BOX_SIZE);
    EXPECT_EQ(sut_->getPosition(), newPosition);
}
